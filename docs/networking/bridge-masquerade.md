...
sequenceDiagram
    ...
    participant VH as virt-handler (特權)
    participant VL as virt-launcher (非特權)
    participant VM as QEMU/VM

    K->>VH: 建立 Pod，指派網路
    VH->>VH: Phase 1：設定 Pod netns（特權操作）
    VH->>VL: 傳遞網路設定快取
    VL->>VL: Phase 2：設定 libvirt XML（非特權）
    VL->>VM: 啟動 QEMU，掛載網路介面
```
...
graph TB
    subgraph "節點 Host"
        subgraph "Pod Network Namespace"
            BR["Linux Bridge<br/>k6t-eth0<br/>IP: 10.244.1.5/24"]
            ETH["eth0<br/>（移除 IP）"]
            TAP["tap0<br/>（連接 VM）"]
            BR --- ETH
            BR --- TAP
        end
        VETH_H["veth-host<br/>（host netns）"]
        ETH <-->|"veth pair"| VETH_H
        VETH_H --- CNI["CNI Plugin<br/>（calico/flannel）"]
    end
    subgraph "VM Guest"
        VM_ETH["eth0<br/>IP: 10.244.1.5/24<br/>（同 Pod IP）"]
    end
    TAP <-->|"virtio tap"| VM_ETH

    style BR fill:#4a9eff,color:#fff
    style TAP fill:#f0ad4e,color:#fff
    style VM_ETH fill:#5cb85c,color:#fff
```
...
graph TB
    subgraph "節點 Host"
        subgraph "Pod Network Namespace"
            NFTABLES["nftables<br/>MASQUERADE / DNAT"]
            ETH["eth0<br/>Pod IP: 10.244.1.5/24"]
            TAP["tap0<br/>Gateway: 10.0.2.1"]
            DHCP["內建 DHCP Server<br/>分配 10.0.2.2 給 VM"]
            NFTABLES --- ETH
            NFTABLES --- TAP
            DHCP --- TAP
        end
    ...
    subgraph "VM Guest"
        VM_ETH["eth0<br/>VM IP: 10.0.2.2/24<br/>GW: 10.0.2.1<br/>DNS: 10.0.2.3"]
    end
    TAP <-->|"virtio tap"| VM_ETH
    Internet["叢集網路<br/>10.244.0.0/16"] <-->|"NAT"| ETH

    style NFTABLES fill:#d9534f,color:#fff
    style DHCP fill:#5bc0de,color:#fff
    style VM_ETH fill:#5cb85c,color:#fff
```
...
graph LR
    subgraph "出站流量（VM → 外部）"
        VM_OUT["VM 10.0.2.2:隨機port"] -->|"POSTROUTING MASQUERADE"| POD_OUT["Pod IP:隨機port"]
        POD_OUT --> CLUSTER["叢集網路"]
    end

    subgraph "入站流量（外部 → VM）"
        SVC_IN["Service/Port-forward"] -->|"PREROUTING DNAT"| VM_IN["VM 10.0.2.2:80"]
    end
```
...
graph TD
    subgraph "Bridge 模式"
        B_P1["Phase 1（特權）<br/>1. 建立 bridge k6t-eth0<br/>2. 移除 eth0 的 Pod IP<br/>3. 設定 bridge IP<br/>4. 設定 ARP proxy"]
        B_P2["Phase 2（非特權）<br/>1. 設定 libvirt XML 使用 bridge<br/>2. 建立 tap 介面<br/>3. 連接 tap 到 bridge<br/>4. 啟動 QEMU"]
        B_P1 --> B_P2
    end

    subgraph "Masquerade 模式"
        M_P1["Phase 1（特權）<br/>1. 設定 nftables MASQUERADE<br/>2. 設定 nftables DNAT（ports）<br/>3. 設定 IPv6 規則（如適用）"]
        M_P2["Phase 2（非特權）<br/>1. 設定 libvirt XML 使用 tap<br/>2. 建立 tap 介面<br/>3. 啟動內建 DHCP server<br/>4. 啟動 QEMU"]
        M_P1 --> M_P2
    end

    subgraph "Passt 模式（對比）"
        P_P1["Phase 1<br/>❌ 不需要（無特權操作）"]
        P_P2["Phase 2（非特權）<br/>1. 啟動 passt 程式（userspace）<br/>2. 設定 libvirt XML 使用 passt<br/>3. 啟動 QEMU"]
        P_P1 -.->|"直接跳過"| P_P2
    end
```
...
sequenceDiagram
    ...
    participant TAP as tap0 介面
    participant DHCP as 內建 DHCP Server
    participant DNS as Cluster DNS (CoreDNS)

    VM->>TAP: DHCP Discover（廣播）
    TAP->>DHCP: 轉發請求
    DHCP->>DHCP: 查詢 Pod DNS 設定<br/>(/etc/resolv.conf)
    DHCP->>TAP: DHCP Offer<br/>IP: 10.0.2.2/24<br/>GW: 10.0.2.1<br/>DNS: 10.0.2.3
    TAP->>VM: 回應
    VM->>TAP: DHCP Request
    DHCP->>TAP: DHCP ACK
    VM->>VM: 設定網路介面完成
    VM->>DNS: DNS 查詢（透過 NAT 轉發到 CoreDNS）
```
