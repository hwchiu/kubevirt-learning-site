...
graph TB
    ...
    subgraph "Control Plane"
        VC[virt-controller]
        API[kube-apiserver]
        VMIM[VirtualMachineInstance\nMigration CR]
    end

    VC -->|建立目標 pod| VL_T
    VC -->|更新 migration 狀態| VMIM
    VH_S -->|監控來源 VMI| QEMU_S
    VH_T -->|監控目標 VMI| QEMU_T
    VH_S -->|通知開始 migration| QEMU_S
    QEMU_S -->|Pre-copy 記憶體傳輸\nMigration Network| QEMU_T
    VH_S & VH_T -->|回報狀態| API
```
...
graph LR
    ...
    VL_S --- MGR_IF_S
    VL_T --- MGR_IF_T
    MGR_IF_S -->|Migration Traffic\n獨立 VLAN/Network| MGR_IF_T
```
...
```mermaid
stateDiagram-v2
    [*] --> Pending : 建立 VirtualMachineInstanceMigration CR
    Pending --> PreparingTarget : 排程成功，找到目標節點
    PreparingTarget --> TargetReady : 目標 virt-launcher pod 就緒
    TargetReady --> Running : 開始傳輸記憶體
    Running --> Succeeded : 最後一批 dirty pages 傳輸完成\nVM 在目標節點恢復執行
    Running --> Failed : 超時 / 傳輸錯誤 / 目標節點失敗
    Running --> Cancelled : 使用者主動取消
    Pending --> Cancelled : 使用者取消
    PreparingTarget --> Failed : 目標 pod 建立失敗
    Succeeded --> [*]
    Failed --> [*]
    Cancelled --> [*]
```
...
```mermaid
flowchart TD
    A[Migration Running] --> B{dirty rate >\ntransfer rate?}
    B -- No --> C[繼續傳輸]
    C --> B
    B -- Yes --> D[降低 vCPU 速度\n例如: 降至 80%]
    D --> E[dirty rate 降低]
...
```mermaid
sequenceDiagram
    participant SRC as 來源節點 VM
    participant NET as Migration Network
    participant DST as 目標節點 VM

    Note over SRC: VM 在來源節點運行
    SRC->>NET: 傳輸少量初始記憶體
    NET->>DST: 初始化目標環境
    Note over SRC,DST: --- 切換點 ---
    SRC-->>DST: VM 控制權轉移到目標節點
    Note over DST: VM 開始在目標節點執行
    Note over DST: 存取未傳輸的記憶體頁面 → page fault
    DST->>NET: 發出 page fault 請求
    NET->>SRC: 從來源節點按需取得頁面
    SRC->>NET: 傳送缺失的記憶體頁面
    NET->>DST: 補足記憶體頁面
    Note over DST: 持續補足，直到所有頁面傳輸完成
    Note over SRC: 來源節點可以退役
```
...
sequenceDiagram
    ...
    participant VM as virt-launcher

    Admin->>K8s: kubectl drain node-1 --ignore-daemonsets
    K8s->>K8s: 為節點加上 NoSchedule taint
    K8s->>K8s: 驅逐節點上的 pods
    K8s->>VC: VMI eviction 請求
    VC->>K8s: 建立 VirtualMachineInstanceMigration CR
    VC->>K8s: 在目標節點建立新的 virt-launcher pod
    K8s->>VH: 通知 virt-handler 開始 migration
    VH->>VM: 啟動 QEMU migration
    Note over VM: Pre-copy 記憶體傳輸中...
    VM-->>K8s: Migration Succeeded
    K8s->>Admin: drain 完成
```
