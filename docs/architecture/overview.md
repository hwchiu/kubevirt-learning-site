...
| 元件 | 部署型態 | 職責摘要 |
|------|---------|---------|
| **[virt-operator](/components/virt-operator)** | Deployment (≥2 副本) | 安裝與管理所有 KubeVirt 元件，監控 `KubeVirt` CR |
| **[virt-api](/components/virt-api)** | Deployment (≥2 副本) | HTTP/WebSocket API 入口，Admission Webhook，console/VNC 代理 |
| **[virt-controller](/components/virt-controller)** | Deployment (≥2 副本) | 叢集級 VM 生命週期管理，建立/刪除 virt-launcher Pod |
| **[virt-handler](/components/virt-handler)** | DaemonSet (每節點一個) | 節點代理，同步 VMI 與 libvirt domain，管理 migration |
| **[virt-launcher](/components/virt-launcher)** | Pod (每 VMI 一個) | 提供 cgroups/namespaces 隔離，執行 libvirtd + QEMU |
