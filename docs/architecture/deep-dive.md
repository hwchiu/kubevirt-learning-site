...
::: info 延伸閱讀
- [系統架構概述](/architecture/overview) — KubeVirt 基礎架構入門
- [VM 生命週期流程](/architecture/lifecycle) — VMI 生命週期詳細流程
- [virt-controller 元件](/components/virt-controller) — 控制器元件詳解
- [virt-handler 元件](/components/virt-handler) — 節點代理詳解
- [Live Migration](/advanced/live-migration) — 遷移機制深入
:::
