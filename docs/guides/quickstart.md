...
```mermaid
graph LR
    A[使用者] -->|virtctl console| B[序列控制台<br/>Serial Console]
    A -->|virtctl vnc| C[圖形桌面<br/>VNC]
    A -->|virtctl ssh| D[SSH 連線]
    A -->|kubectl port-forward| E[自訂 Port<br/>Forwarding]
```
...
```mermaid
graph LR
    VM[VM<br/>10.0.2.2] -->|NAT| Pod[Pod Network<br/>10.244.x.x] -->|CNI| Node[節點網路] -->|路由| External[外部網路]
```
...
```mermaid
graph TD
    A[安裝 KubeVirt Operator] --> B[部署 kubevirt CR]
    B --> C[安裝 virtctl CLI]
    C --> D{選擇 VM 類型}
    D -->|Linux| E[建立 Linux VM YAML]
    D -->|Windows| F[建立 Windows VM YAML]
    E --> G[kubectl apply]
    F --> G
    G --> H[等待 VM Running]
    H --> I{連線方式}
    I -->|文字| J[virtctl console]
    I -->|圖形| K[virtctl vnc]
    I -->|SSH| L[virtctl ssh]
    H --> M[日常操作]
    M --> N[start / stop / restart]
    M --> O[migrate / snapshot / clone]
    M --> P[添加磁碟 / 設定網路]
```
