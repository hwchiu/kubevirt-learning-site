...
```mermaid
flowchart TD
    PVC[PVC Access Mode] --> RWO[RWO - ReadWriteOnce]
    PVC --> RWX[RWX - ReadWriteMany]
    PVC --> ROX[ROX - ReadOnlyMany]

    RWO --> RWOF1[✅ VM 正常啟動]
    RWO --> RWOF2[❌ Live Migration 不支援]
    RWO --> RWOF3[✅ 適合大多數單節點場景]

    RWX --> RWXF1[✅ VM 正常啟動]
    RWX --> RWXF2[✅ Live Migration 完整支援]
    RWX --> RWXF3[⚠️ 需要 CSI 驅動支援 RWX]
    RWX --> RWXF4[💡 NFS、Ceph RBD、Longhorn 等支援]

    ROX --> ROXF1[✅ VM 啟動（Ephemeral 模式）]
    ROX --> ROXF2[❌ 無法直接作為讀寫磁碟]
    ROX --> ROXF3[✅ 配合 Ephemeral 做寫入 overlay]
```
...
DataVolume（宣告）
    │
    ├── 建立 PVC（實際儲存）
    │       └── StorageClass 決定的後端 PV
    │
    └── 建立 Importer/Cloner Pod（資料搬運）
            ├── HTTP Importer：從 URL 下載映像
            ├── S3 Importer：從 S3 下載映像
            ├── PVC Cloner：從現有 PVC 克隆
            ├── Registry Importer：從 ContainerDisk 映像轉換
            └── Upload Server：等待 virtctl 上傳
```
...
原始 PVC（唯讀，不修改）
      ...
      │ (backing file)
      ▼
QEMU qcow2 overlay（在 emptyDir 中，暫態）
      │
      │ (VM 的讀寫都在這裡)
      ▼
  Guest OS（VM）
...
```
VM 建立 → 自動建立 DataVolume → CDI 匯入資料 → VM 啟動
    │
    └── VM 刪除 → 自動刪除 DataVolume → 自動刪除 PVC
```
