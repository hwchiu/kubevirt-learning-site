...
sequenceDiagram
    ...
    participant D as drain.RunNodeDrain
    participant P as Pod (Eviction API)

    C->>N: 加入 Taint（NoSchedule / NoExecute）
    C->>N: Cordon（設 spec.unschedulable=true）
    C->>D: 呼叫 drain.RunNodeDrain(drainer, nodeName)
    loop 對每個需驅逐的 Pod
        D->>P: POST /pods/{name}/eviction
        P-->>D: 202 Accepted 或 429 Too Many Requests（PDB 阻擋）
        D-->>D: 等待 Pod 消失（最多 Timeout）
    end
    D-->>C: 回傳錯誤（若超時或部分失敗）
    C->>C: 更新 NodeMaintenance status（drainProgress、conditions）
```
