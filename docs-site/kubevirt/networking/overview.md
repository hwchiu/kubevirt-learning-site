...
```mermaid
flowchart TD
    A[virt-launcher Pod 啟動] --> B[Phase 1: Privileged Setup]
    B --> B1[修改 Linux Network Namespace]
    B --> B2[建立 Linux Bridge / TAP 設備]
    B --> B3[設定 iptables / nftables 規則]
    B --> B4[設定 IP 地址和路由]
    B1 & B2 & B3 & B4 --> C[Phase 2: Unprivileged Setup]
    C --> C1[設定 QEMU 網路裝置]
    C --> C2[建立 virtio-net / e1000 虛擬 NIC]
    C --> C3[連接 TAP 設備到 QEMU]
    C1 & C2 & C3 --> D[VM 啟動，網路就緒]
```
...
graph TB
    subgraph "Kubernetes Cluster"
        ...
        subgraph "Node 2"
            subgraph "virt-launcher Pod 2"
                VM2["🖥️ VM 2\n(SR-IOV)"]
                VF["SR-IOV VF\n直通"]
                VM2 <-->|PCIe pass-through| VF
            end
        end
