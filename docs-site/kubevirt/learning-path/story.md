...
> 跟著阿龍，從接到「把公司 VMware VM 搬進 K8s」這個任務開始，一步步理解 KubeVirt 的世界。
> 每個章節都是阿龍真實遭遇的問題——你會看到他怎麼查文件、怎麼犯錯、怎麼一點一點把拼圖拼起來。
...
> ### 📚 去這裡深入了解
> 阿龍的那個疑問——「K8s 本來就能跑 Container，為什麼還需要 KubeVirt？」——在這裡有完整的答案：
>
...
阿龍打開 KubeVirt 的網路文件，發現光是 interface type 就有好幾種：`masquerade`、`bridge`、`sr-iov`……他看了五分鐘，腦袋開始打結。
...
資安工程師 Julie 找到阿龍，打開一份報告：「你知道 virt-launcher Pod 用了哪些 capability 嗎？我看到 `SYS_PTRACE`、`NET_ADMIN`，在我們的安全政策下這些都是 high risk。」
...
> ### 📚 想繼續往更深處走？
>
> 阿龍的故事到這裡告一段落，但 KubeVirt 還有很多值得探索的角落。
> 以下是按主題整理的深入閱讀清單——阿龍在六個月裡逐漸把這些都翻過了一遍：
>
> **架構與原始碼**
...
> ---
>
> 阿龍從「K8s 不是跑 Container 的嗎？」到「讓我來規劃一下 migration 策略」，走了六個月。
> 你可以走得更快——因為你現在有地圖了。
