...
```mermaid
flowchart TD
    U([使用者]) -->|kubectl apply VM YAML| API[Kubernetes API Server]

    subgraph "控制平面 (Control Plane)"
        API -->|VM 建立事件| VC[virt-controller\nVM Controller]
        VC -->|建立 VMI 物件| API
        API -->|VMI 建立事件| VC2[virt-controller\nVMI Controller]
        VC2 -->|建立 virt-launcher Pod| API
    end

    subgraph "節點 (Node)"
        API -->|Pod 排程完成| VH[virt-handler]
        VH -->|透過 gRPC SyncVMI\n呼叫 virt-launcher| VL[virt-launcher]

        subgraph "virt-launcher 初始化序列"
            VL --> GCC[generateConverterContext\n蒐集節點拓撲、設備資訊]
            GCC --> CONV[Convert\nVMI Spec → Domain XML]
            CONV --> PSH[preStartHook\n執行啟動前準備]
            PSH --> CI[生成 CloudInit ISO\nor Sysprep 磁碟]
            CI --> HOOK[呼叫 Hook Sidecars]
            HOOK --> EXPAND[展開磁碟映像]
            EXPAND --> DEFINE[libvirt.DomainDefine\n定義 Domain]
            DEFINE --> START[libvirt.DomainCreate\n啟動 QEMU 行程]
        end
    end

    START -->|VMI status: Running| API
```
...
```mermaid
flowchart LR
    VMI[VMI Spec\nkubevirt.io/v1] -->|輸入| GCC

    subgraph "generateConverterContext()"
        GCC[蒐集執行環境資訊]
        GCC --> NI[節點拓撲\nNUMA / CPU]
        GCC --> DC[設備設定\n/proc/cmdline\nKVM capabilities]
        GCC --> SR[SR-IOV 設備]
    ...
    GCC -->|ConverterContext| CV

    subgraph "Convert(vmi, ctx)"
        CV[初始化 Domain 結構] --> CPU_C[Convert CPU\nvCPU數量 / Topology\nCPU Model / Features]
        CV --> MEM_C[Convert Memory\nHugepages / NUMA]
        CV --> DSK_C[Convert Disks\n磁碟 + 映像來源]
        CV --> NET_C[Convert Networks\nvirtio / e1000 / SR-IOV]
        CV --> CLK_C[Convert Clock\nRTC / Hypervisor Timer]
        CV --> FW_C[Convert Firmware\nBIOS / EFI / Secure Boot]
        CV --> VID_C[Convert Video\nVGA / Virtio-GPU]
        CV --> SND_C[Convert Sound\nAC97 / ICH9]
        CV --> FEA_C[Convert Features\nACPI / APIC / SMM]
    end

    CV -->|libvirt Domain XML| LIBVIRT[libvirtd]
```
...
```mermaid
flowchart TD
    CONV["Convert(vmi, c *ConverterContext)"] --> D1
    CONV --> D2
    CONV --> D3
    CONV --> D4
    CONV --> D5

    D1["Convert_v1_Disk_To_api_Disk()\n處理每一顆磁碟\n決定 bus / target / driver"]
    D2["Convert_v1_Interface_To_api_Interface()\n網路介面 Model 轉換"]
    D3["Convert_v1_CPU_To_api_CPU()\nvCPU 數量 + Topology\nCPU Pinning"]
    D4["Convert_v1_Memory_To_api_Memory()\nHugepages / NUMA"]
    D5["convertDisksToStorages()\n將磁碟對應到實際儲存後端"]

    D1 --> BUS["根據 bus 類型設定 target:\nvirtio → vdX\nsata → sdX\nscsi → sdX\nusb → sdX"]
    D1 --> BO["BootOrder 對應\ndisk.bootOrder → domain bootOrder"]
```
...
sequenceDiagram
    ...
    participant IC as container-disk\nInit Container
    participant VL as virt-launcher\n主容器
    participant QEMU

    K8s->>IC: 啟動 Init Container
    Note over IC: 從 OCI Registry 拉取映像
    IC->>IC: 掛載 OCI 映像層
    IC->>IC: 找到 /disk/disk.img\nor /disk/disk.qcow2
    IC->>IC: 複製到 emptyDir\n/var/run/kubevirt/container-disks/
    IC-->>K8s: Init Container 完成

    K8s->>VL: 啟動 virt-launcher
    VL->>VL: 偵測 emptyDir 中的磁碟
    VL->>VL: Convert() 設定磁碟路徑
    VL->>QEMU: 啟動並掛載磁碟
```
...
```mermaid
flowchart TD
    PSH[preStartHook 執行] --> CIS[generateCloudInitISO]
    CIS --> SRC{資料來源}

    SRC -->|cloudInitNoCloud.userData| UD[userData 字串\n直接寫入]
    SRC -->|cloudInitNoCloud.userDataSecretRef| UDS[從 Kubernetes Secret\n讀取 userData]
    SRC -->|cloudInitNoCloud.userDataBase64| UDB[Base64 解碼\nuserData]
    SRC -->|networkDataSecretRef| NDS[從 Secret 讀取\nnetworkData]

    UD & UDS & UDB --> MK[mkisofs / genisoimage\n產生 NoCloud ISO]
    NDS --> MK

    MK --> ISO[/var/run/kubevirt-private/\nvmi-disks/cloudinitnocloud/\nnoCloud.iso]
    ISO --> QEMU[掛載到 VM 作為 CD-ROM]
```
...
sequenceDiagram
    ...
    U->>API: 建立包含 unattend.xml 的 ConfigMap
    U->>API: 建立 VMI 引用此 ConfigMap
    API->>VL: SyncVMI
    VL->>VL: Convert_v1_SysprepSource_To_api_Disk()
    Note over VL: 將 ConfigMap 內容\n掛載為 virtio-fs 或 ISO
    VL->>W: 啟動 Windows VM
    W->>W: 偵測 Sysprep 磁碟
    W->>W: 執行 sysprep /unattend
    W->>W: 首次開機設定完成
```
...
flowchart LR
    OVMF[OVMF_VARS.fd\n模板檔案] -->|首次啟動複製| NVRAM
    subgraph "virt-launcher Pod"
        NVRAM[/var/run/kubevirt-private/\nlibvirt/qemu/nvram/\nvmi-name_VARS.fd]
    end
    NVRAM -->|掛載| QEMU[QEMU\nEFI 運行時]
    NVRAM -->|重啟保留| PVC[PVC\n或 hostPath]
```
