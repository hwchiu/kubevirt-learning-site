...
graph TB
    subgraph ControllerQuorum["Controller Quorum（Raft）"]
        ...
        C1 <-->|"Raft 選舉 / Log 複製"| C2
        C2 <-->|"Raft 選舉 / Log 複製"| C3
        C1 <-->|"Raft 選舉 / Log 複製"| C3
    end

    subgraph BrokerCluster["Broker 叢集"]
        B1["Broker 1"]
        B2["Broker 2"]
        B3["Broker 3"]
    end
    ...
    C1 -->|"MetadataUpdate (Fetch)"| B1
    C1 -->|"MetadataUpdate (Fetch)"| B2
    C1 -->|"MetadataUpdate (Fetch)"| B3
...
```mermaid
flowchart LR
    Network["網路層<br/>(Selector / KafkaChannel)"] -->|"請求入隊"| RequestQueue["RequestQueue<br/>(ArrayBlockingQueue)"]
    RequestQueue -->|"分配給 Handler"| KRH["KafkaRequestHandlerPool<br/>(N 個執行緒)"]
    KRH --> KApis["KafkaApis<br/>(請求分派)"]
    KApis -->|"ProduceRequest"| ReplicaManager
    KApis -->|"FetchRequest"| ReplicaManager
    KApis -->|"OffsetFetch"| GroupCoordinator
    KApis -->|"JoinGroup"| GroupCoordinator
    KApis -->|"CreateTopics"| AdminManager
    ReplicaManager --> LogManager["LogManager<br/>(本地 Log 管理)"]
    LogManager --> LogDir["磁碟<br/>(Log Segments)"]
    ReplicaManager -->|"副本複製 (Fetch)"| OtherBroker["其他 Broker<br/>(Follower)"]
```
...
graph LR
    subgraph Before["Compaction 前"]
        A1["k=A, v=1"] --> A2["k=B, v=2"] --> A3["k=A, v=3"] --> A4["k=C, v=4"] --> A5["k=B, v=5"]
    end
    subgraph After["Compaction 後"]
        B1["k=A, v=3"] --> B2["k=C, v=4"] --> B3["k=B, v=5"]
    end
    Before --> After
```
...
sequenceDiagram
    ...
    Producer->>Leader: ProduceRequest (acks=-1)
    Leader->>Leader: 寫入本地 Log
    F1->>Leader: FetchRequest (Follower 複製)
    Leader-->>F1: FetchResponse (新訊息)
    F1->>F1: 寫入本地 Log
    F2->>Leader: FetchRequest
    Leader-->>F2: FetchResponse
    F2->>F2: 寫入本地 Log
    Note over Leader: 所有 ISR 成員確認後<br/>更新 High Watermark
    Leader-->>Producer: ProduceResponse (offset)
```
