...
```mermaid
graph LR
    subgraph Source["Source 叢集"]
        SB["Broker (Source)"]
    end

    subgraph MM2["MirrorMaker 2 Worker"]
        MirrorSource["MirrorSourceConnector<br/>（複製 Topic 資料）"]
        MirrorCheckpoint["MirrorCheckpointConnector<br/>（複製 Consumer 位移）"]
        MirrorHeartbeat["MirrorHeartbeatConnector<br/>（心跳監控）"]
    end

    subgraph Target["Target 叢集"]
        TB["Broker (Target)"]
    end

    SB -->|"消費"| MirrorSource
    MirrorSource -->|"生產"| TB
    SB -->|"讀取 Offset"| MirrorCheckpoint
    MirrorCheckpoint -->|"寫入轉換後 Offset"| TB
    MirrorHeartbeat -->|"心跳"| TB
```
...
```mermaid
graph LR
    Producer -->|"1. 向 Schema Registry 登記 Schema"| SR["Schema Registry"]
    SR -->|"2. 回傳 Schema ID"| Producer
    Producer -->|"3. 發送: [Magic Byte][Schema ID][Avro Payload]"| Kafka
    Consumer -->|"4. 消費訊息"| Kafka
    Consumer -->|"5. 從 Schema Registry 取得 Schema"| SR
    SR -->|"6. 回傳 Schema"| Consumer
    Consumer -->|"7. 使用 Schema 反序列化"| Consumer
```
